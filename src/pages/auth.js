...
const Login = () => {
  ...
  const onSubmit = async (event) => {
    ...
    try {
      const response = await axios.post("https://mern-recipe-app.onrender.com/auth/login", { //////////////////// CHANGE IT AFTER DEPLOYMENT
        username,
        password,
      });
...
const Register = () => {
  ...
  const onSubmit = async (event) => {
    ...
    try {
      await axios.post("https://mern-recipe-app.onrender.com/auth/register", {
        username,
        password,
  ...
  return (
    <div className="bg-gray-300 rounded flex flex-col justify-center mx-10 py-12 lg:px-8">
      <form className="flex flex-col items-center" onSubmit={onSubmit}>
        ...
        <button
          type="submit"
          className="bg-blue-500 mt-6 self-auto hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
        >
          {label}
        </button>
      </form>
